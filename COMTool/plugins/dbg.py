...
""".format(
	...
	def onWidgetSettings(self, parent):
		...
		self.receiveSettingsAscii.setToolTip(
			_(
				"Show recived data as visible format, select decode method at top right corner"
			)
		)
		...
		self.receiveSettingsAutoLinefeed.setToolTip(
			_("Auto linefeed after interval, unit: ms")
		)
		self.receiveSettingsAutoLinefeedTime = QLineEdit("200")
		self.receiveSettingsAutoLinefeedTime.setProperty("class", "smallInput")
		self.receiveSettingsAutoLinefeedTime.setToolTip(
			_("Auto linefeed after interval, unit: ms")
		)
		...
		self.receiveSettingsTimestamp.setToolTip(
			_(
				"Add timestamp before received data, will automatically enable auto line feed"
			)
		)
		self.receiveSettingsColor = QCheckBox(_("Color"))
		self.receiveSettingsColor.setToolTip(
			_("Enable unix terminal color support, e.g. \\33[31;43mhello\\33[0m")
		)
		self.receiveSettingsWrap = QCheckBox(_("Display wrap"))
		self.receiveEscape = QCheckBox(_("Escape"))
		self.receiveEscape.setToolTip(
			_("Enable escape characters support like \\t \\r \\n \\x01 \\001")
		)
		self.receiveSettingsWrap.setToolTip(
			_(
				"When content in a line is too long, always auto wrap to show, and no scroll bar"
			)
		)
		...
		serialReceiveSettingsLayout.addWidget(
			self.receiveSettingsAutoLinefeed, 2, 0, 1, 1
		)
		serialReceiveSettingsLayout.addWidget(
		...
		self.sendSettingsAscii.setToolTip(
			_(
				"Get send data as visible format, select encoding method at top right corner"
			)
		)
		...
		serialSendSettingsLayout.addWidget(
			self.sendSettingsScheduledCheckBox, 2, 0, 1, 1
		)
		...
		self.sendSettingsAppendNewLine.clicked.connect(
			lambda: self.bindVar(
				self.sendSettingsAppendNewLine, self.config, "sendAutoNewline"
			)
		)
		self.sendSettingsEscape.clicked.connect(
			lambda: self.bindVar(self.sendSettingsEscape, self.config, "sendEscape")
		)
		self.sendSettingsCRLF.clicked.connect(
			lambda: self.bindVar(self.sendSettingsCRLF, self.config, "useCRLF")
		)
		self.receiveSettingsColor.clicked.connect(self.onSetColorChanged)
		self.receiveSettingsAutoLinefeedTime.textChanged.connect(
			lambda: self.bindVar(
				self.receiveSettingsAutoLinefeedTime,
				self.config,
				"receiveAutoLindefeedTime",
				vtype=int,
				vErrorMsg=_("Auto line feed value error, must be integer"),
				emptyDefault="200",
			)
		)
		self.sendSettingsScheduled.textChanged.connect(
			lambda: self.bindVar(
				self.sendSettingsScheduled,
				self.config,
				"sendScheduledTime",
				vtype=int,
				vErrorMsg=_("Timed send value error, must be integer"),
				emptyDefault="300",
			)
		)
		self.sendSettingsScheduledCheckBox.clicked.connect(
			lambda: self.bindVar(
				self.sendSettingsScheduledCheckBox, self.config, "sendScheduled"
			)
		)
		self.receiveSettingsWrap.clicked.connect(self.onSettingWrap)
		self.receiveEscape.clicked.connect(
			lambda: self.bindVar(self.receiveEscape, self.config, "receiveEscape")
		)
		return widget
	...
	def onWidgetFunctional(self, parent):
		...
		self.saveLogAutoNew.setToolTip(
			_("When start a new connection, will automatically create a new log file")
		)
		...
		self.saveLogAutoNew.clicked.connect(
			lambda: self.bindVar(self.saveLogAutoNew, self.config, "saveLogAutoNew")
		)
	...
	def onUiInitDone(self):
		...
		# wrap
		self.receiveArea.setLineWrapMode(
			QTextEdit.WidgetWidth if paramObj["wrap"] else QTextEdit.NoWrap
		)
		self.sendArea.setLineWrapMode(
			QTextEdit.WidgetWidth if paramObj["wrap"] else QTextEdit.NoWrap
		)
		# send items
	...
	def onSendSettingsAsciiClicked(self):
		...
		try:
			...
			self.sendArea.clear()
			if data != "":
				data = utils.hex_str_to_bytes(data).decode(
					self.configGlobal["encoding"], "ignore"
				)
				self.sendArea.insertPlainText(data)
	...
	def onAutoLinefeedClicked(self):
		if (
			self.config["showTimestamp"] or self.config["recordSend"]
		) and not self.receiveSettingsAutoLinefeed.isChecked():
			self.receiveSettingsAutoLinefeed.setChecked(True)
			self.hintSignal.emit(
				"warning",
				_("Warning"),
				_("linefeed always on if timestamp or record send is on"),
			)
		self.config["receiveAutoLinefeed"] = (
			self.receiveSettingsAutoLinefeed.isChecked()
		)
	...
	def onSent(self, ok, msg, length, path):
		if ok:
			...
			self.hintSignal.emit(
				"error", _("Error"), _("Send data failed!") + " " + msg
			)
	...
	def onSentFile(self, ok, msg, length, path):
		...
		if ok:
			...
			self.hintSignal.emit(
				"error", _("Error"), _("Send file failed!") + " " + msg
			)
	...
	def selectFile(self):
		...
		fileName_choose, filetype = QFileDialog.getOpenFileName(
			self.mainWidget, _("Select file"), oldPath, _("All Files (*)")
		)
	...
	def onLog(self, text):
		path = (
			self.config["saveLogPath2"]
			if self.config["saveLogAutoNew"]
			else self.config["saveLogPath"]
		)
		if self.config["saveLog"] and path:
			with open(
				path, "a+", encoding=self.configGlobal["encoding"], newline="\n"
			) as f:
				f.write(text)
	...
	def insertSendItem(self, text="", load=False):
		...
		cmd.textChanged.connect(
			lambda: self.onCustomItemChange(
				self.customSendItemsLayout.indexOf(item), cmd, send
			)
		)
		send.setProperty("class", "smallBtn")
		send.clicked.connect(
			lambda: self.sendCustomItem(
				self.config["customSendItems"][self.customSendItemsLayout.indexOf(item)]
			)
		)
		delete = QPushButton("")
		...
		delete.clicked.connect(
			lambda: self.deleteSendItem(self.customSendItemsLayout.indexOf(item), item)
		)
	...
	def sendFile(self):
		...
		if not self.isConnected():
			...
			self.send(
				file_path=filename,
				callback=lambda ok, msg, length, path: self.onSentFile(
					ok, msg, length, path
				),
			)
		...
		try:
			if self.isConnected():
				...
				if self.config["recordSend"]:
					...
					isHexStr, sendStr, sendStrsColored = self.bytes2String(
						data,
					...
					self.receiveUpdateSignal.emit(
						head, [sendStr], self.configGlobal["encoding"]
					)
	...
	def onSendData(self, call=True, data=None):
		try:
			...
			self.hintSignal.emit(
				"error", _("Error"), _("get data error") + ": " + str(e)
			)
		...
		if datas:
			...
			for data in datas:
				if type(data) == str:
					...
					self.receiveArea.insertPlainText(
						data.decode(encoding=encoding, errors="ignore")
					)
			
			# 如果长度太长, 先清空 -- by hench
			if self.statusBar.getRxCount() > 50000:
				print("接收长度太长, 先清空 -- by hench")
		...
		if decoding:
			...
			for color, bg, text in coloredText:
				if self.config["receiveEscape"]:
					...
					decodedColoredText.append(
						[color, bg, text.decode(encoding=decoding, errors="ignore")]
					)
	...
	def bytes2String(self, data: bytes, showAsHex: bool, encoding="utf-8"):
		...
		try:
			dataPlain, dataColore, remain = self.getColoredText(
				data, self.configGlobal["encoding"]
			)
			if remain:
				dataPlain += remain.decode(
					encoding=self.configGlobal["encoding"], errors="ignore"
				)
		except Exception:
	...
	def receiveDataProcess(self):
		...
		while not self.receiveProgressStop:
			...
			if (
				time.time() - timeLastReceive
				> self.config["receiveAutoLindefeedTime"] / 1000
				or self.justSent
			):
			...
			if len(buffer) > 0:
				...
				# show as string, and need to render color, wait for \n or until timeout to ensure color flag in buffer
					...
					if (
						...
						data, colorData, remain = self.getColoredText(
							buffer, self.configGlobal["encoding"]
						)
						buffer = remain
				...
				if data:
					...
					if new_line:
						timeNow = "[{}] ".format(
							utils.datetime_format_ms(datetime.now())
						)
						if self.config["recordSend"]:
							head += "<= "
						if self.config["showTimestamp"]:
						...
						if hexstr:
							head += "[HEX] "
						if (
							self.config["recordSend"] or self.config["showTimestamp"]
						) and not head.endswith("<= "):
							head = head[:-1] + ": "
						new_line = False
					self.receiveUpdateSignal.emit(
						head, [colorData], self.configGlobal["encoding"]
					)
					logData = head + data
			if len(new) > 0:
