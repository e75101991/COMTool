...
# class Graph_DragTof(Graph_Widget_Base):
...
class Graph_MetaSenseLite(Graph_Widget_Base):
    updateSignal = pyqtSignal(dict)
    id = "MetaSenseLite"
    ...
    def __init__(
        self,
        parent=None,
        hintSignal=lambda type, title, msg: None,
        rmCallback=lambda widget: None,
        send=lambda x: None,
        config=None,
    ):
        default = {"xRange": 10, "xRangeEnable": True, "header": "\\x00\\xFF"}
        super().__init__(
            parent,
        ...
        self.imv.setColorMap(
            pg.ColorMap(pos=np.linspace(0.0, 1.0, len(jetcolors)), color=jetcolors)
        )
        ...
        rmBtn.clicked.connect(self.remove)
        self.capTargetBtn.clicked.connect(
            lambda: self.capTargetBtn.setText(
                QFileDialog.getExistingDirectory(
                    None, "Choose Directory To Save", os.path.expanduser("~")
                )
                or self.capTargetBtn.text()
            )
        )
        ...
        ispRun.toggled.connect(lambda x: self.sendCmd("AT+ISP=%1d\r" % (x)))

        def dispChanged():
            self.sendCmd(
                "AT+DISP=%1d\r"
                % (
                    1 * dispCbLcd.isChecked()
                    + 2 * dispCbUsb.isChecked()
                    + 4 * dispCbUart.isChecked()
                )
            )
        ...
        antiMmiCb.toggled.connect(lambda x: self.sendCmd("AT+ANTIMMI=%1d\r" % (x)))
        ...
        unitSlider.valueChanged.connect(
            lambda i: (
                unitLabel.setText("Unit: %-5s" % ("auto" if i == 0 else str(i))),
                self.sendCmd("AT+UNIT=%d\r" % (i)),
            )
        )
        ...
        fpsSlider.valueChanged.connect(
            lambda i: (
                fpsLabel.setText(" Fps: %-5d" % (i)),
                self.sendCmd("AT+FPS=%d\r" % (i)),
            )
        )
        ...
        evSlider.valueChanged.connect(
            lambda i: (
                evLabel.setText("  Ev: %-5s" % ("AE" if i == 0 else str(i))),
                self.sendCmd("AT+AE=%d\r" % (1 if i == 0 else 0)),
                self.sendCmd("AT+EV=%d\r" % (i)),
            )
        )
        evSlider.setValue(0)
    ...
    def decodeData(self, data: bytes):
        ...
        frameData = [
            unpack("B", frame[20 + i : 21 + i])[0] for i in range(0, frameDataLen, 1)
        ]
        ...
        queueData = {"frameID": frameID, "res": res, "frameData": frameData}
        self.queue.put(queueData)
        ...
        if not self.queue.empty():
            ...
            if self.onceShot:
                ...
                fname = "%d-unit%d" % (
                    ...
                    self.unitSlider.value(),
                )
                Image.fromarray(arr).save(os.path.join(DownloadsPATH, fname + ".bmp"))
            ...
            if (
                len(self.frames) != 0
                and self.frames[len(self.frames) - 1].shape != arr.T.shape
            ):
                self.frames.clear()
            ...
            if self.focus[0].isChecked():
                ...
                self.focus[2].setText(
                    str(
                        compute_real_distance(
                            np_arrs[len(self.frames) - 1][
                                int(self.focus[1][0].text()),
                                int(self.focus[1][1].text()),
                            ],
                            self.unitSlider.value(),
                        )
                    )
                ...
                def draw_reticle_and_info(buf, x, y):
                    ll = buf.shape[0] // 25
                    u, d, l, r = (
                        min(ll * 25, max(0, y - ll)),
                        max(0, min(ll * 25, y + ll + 1)),
                        min(ll * 25, max(0, x - ll)),
                        max(0, min(ll * 25, x + ll + 1)),
                    )
                    buf[u:d, x] = 0x00
                    buf[y, l:r] = 0x00

                draw_reticle_and_info(
                    np_arrs[len(self.frames) - 1],
                    int(self.focus[1][0].text()),
                    int(self.focus[1][1].text()),
                )
